...
function renderYearView() {
    ...
    for (let month = 0; month < 12; month++) {
        ...
        html += `
            <div onclick="app.setActiveMonth(${month})" class="bg-white/60 backdrop-blur-md rounded-3xl p-6 border border-slate-200/60 month-card-enter hover:bg-white hover:shadow-xl hover:-translate-y-1 hover:border-blue-200 hover:ring-2 hover:ring-blue-50 transition-all duration-300 cursor-pointer group flex flex-col items-center justify-center text-center h-48" style="animation-delay: ${delay}s">
                <h3 class="text-2xl font-bold text-slate-800 tracking-tight group-hover:text-blue-600 transition-colors">${MONTHS[month]}</h3>
                <p class="text-sm font-medium text-slate-400 mt-1">${assignedCount > 0 ? `<span class="text-blue-500 font-bold">${assignedCount}</span> nöbet girildi` : 'Planlanmadı'}</p>
                <div class="mt-5 px-5 py-2.5 rounded-xl bg-blue-50 text-blue-600 text-sm font-bold opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300">
                    Takvimi Yönet <i class="ph ph-arrow-right ml-1"></i>
                </div>
            </div>
        `;
    }
...
function renderMonthView(month) {
    ...
    let html = `
        <div class="animate-[slideUpFade_0.3s_ease-out]">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
                <button onclick="app.setActiveMonth(null)" class="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/70 backdrop-blur-md border border-slate-200 shadow-sm hover:shadow hover:bg-white text-slate-600 font-medium transition-all hover:-translate-x-1 active:scale-95">
                    <i class="ph ph-arrow-left text-lg"></i> ${appState.activeYear} Yılına Dön
                </button>
                <button onclick="app.clearCurrentMonth()" class="flex flex-1 sm:flex-none items-center justify-center gap-2 px-4 py-2 rounded-xl bg-rose-50/80 backdrop-blur-md border border-rose-200 shadow-sm hover:shadow hover:bg-rose-100/80 hover:border-rose-300 text-rose-600 font-medium transition-all active:scale-95 group">
                    <i class="ph ph-trash text-lg group-hover:-rotate-12 transition-transform"></i> Seçili Ayı Temizle
                </button>
            </div>
            <div class="bg-white/80 backdrop-blur-xl border border-white/60 p-6 md:p-10 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_40px_rgb(0,0,0,0.06)] transition-shadow">
